...
export function ProductModal({
    ...
    recPages > 1 ? (
      <div className="flex justify-center items-center gap-1.5 pt-2">
        {Array.from({ length: recPages }).map((_, i) => (
          <span key={i} className={`h-1.5 rounded-full transition-all ${i === recPage ? 'w-5 bg-[#C09E6D]' : 'w-1.5 bg-[#E6DFD5]'}`} />
        ))}
      </div>
    ) : null;

  useEffect(() => {
    const update = () => { if (recScrollRef.current) updateRecDots(recScrollRef.current); };
  ...
  useEffect(() => {
    ...
    ro.observe(row);
    document.fonts?.ready?.then(() => { measure(); update(); }).catch(() => {});
  ...
  const renderDescription = () =>
    product && getProductDesc(product) ? (
      <div><p className="text-[#4A3B32] text-sm sm:text-base leading-relaxed">{getProductDesc(product)}</p></div>
    ) : null;

  const renderIngredients = () =>
    ingredients.length > 0 ? (
      <div className="pt-2">
        <span className="block font-bold uppercase tracking-wider text-[#8E7A68] text-xs mb-2">{t('ingredients')}</span>
        <ul className="space-y-1.5 pl-0.5">
          {ingredients.map((ingredient, idx) => (
            <li key={idx} className="flex items-start gap-2.5 text-[#4A3B32] text-sm sm:text-base leading-relaxed">
  ...
  const renderRecommended = () =>
    recommendedProducts.length > 0 ? (
      <div className="pt-3 border-t border-[#E6DFD5]/60">
        <span className="block font-bold uppercase tracking-wider text-[#8E7A68] text-sm mb-2.5">{t('recommendedWith')}</span>
        <div
          ref={(node) => { if (node) recScrollRef.current = node; }}
          onPointerDown={onRecPointerDown} onPointerLeave={onRecPointerUp} onPointerUp={onRecPointerUp} onPointerMove={onRecPointerMove}
        ...
        >
          {recommendedProducts.map((rec) => (
            <button key={rec.id} onClick={() => { if (!recIsDragging) onOpenProduct(rec); }}
            ...
            >
              ...
              <div className="p-2 w-full">
                <h4 className="font-sans font-medium text-xs text-[#231913] leading-snug line-clamp-2 pointer-events-none">{getProductName(rec)}</h4>
              </div>
            </button>
          ))}
        </div>
  ...
  const renderFooter = () => (
    <>
      <div className="flex items-center bg-[#F1ECE3] border border-[#E6DFD5] rounded-full overflow-hidden shrink-0 h-12 shadow-sm">
        <button onClick={onDecrementQty} disabled={modalQty <= 1}
          className={`w-10 h-12 flex items-center justify-center text-[#3E2F26] transition-all ${modalQty <= 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-[#E6DFD5] active:scale-95'}`} aria-label="Decrease quantity"
        ><Minus className="w-4 h-4" /></button>
        <span className="w-8 text-center font-bold text-base text-[#231913] select-none lining-nums">{modalQty}</span>
  ...
  const sharedBackdrop = (
    <motion.div
      key={isWide ? 'overlay-wide' : 'overlay-mobile'}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
  ...
  const cartButtonBlock = cartButtonRect && (
    <div style={{ top: cartButtonRect.top, right: cartButtonRect.right, height: cartButtonRect.height }}
      className="fixed z-[60] flex items-center justify-end gap-2 sm:gap-2.5 pointer-events-none"
    >
      <AnimatePresence>
        {cartToast && (
          <motion.div key={cartToast.id}
          ...
          >
            <div className="w-5 h-5 rounded-full bg-[#C09E6D] text-white flex items-center justify-center shrink-0 shadow-xs"><Check className="w-3.5 h-3.5 stroke-[3]" /></div>
            <span className="font-sans font-medium text-xs sm:text-sm text-[#FAF6EE] whitespace-nowrap">{cartToast.message}</span>
            {cartToast.qty > 1 && <span className="font-sans font-bold text-xs bg-[#C09E6D]/30 border border-[#C09E6D]/50 text-[#FAF6EE] px-2 py-1 rounded-full leading-tight">+{cartToast.qty}</span>}
          </motion.div>
        )}
      </AnimatePresence>
      <motion.button onClick={(e) => { e.stopPropagation(); onOpenCart(); }}
        animate={bouncingCart ? { scale: [1, 1.25, 0.92, 1.12, 1], rotate: [0, -8, 8, -4, 0] } : { scale: 1, rotate: 0 }}
        transition={{ duration: 0.45, ease: 'easeOut' }}
        className="pointer-events-auto relative flex items-center justify-center w-10 h-10 rounded-full bg-[#C09E6D] hover:bg-[#ad8b5b] text-white shadow-md active:scale-90 transition-all cursor-pointer border border-[#FAF6EE]/40 shrink-0" aria-label={t('cart')}
      >
        <ConciergeBell className="w-5 h-5 text-white stroke-[2.2]" />
        {totalCartItemsCount > 0 && (
          <motion.span key={totalCartItemsCount} initial={{ scale: 0.4 }} animate={{ scale: 1 }}
          ...
          >{totalCartItemsCount}</motion.span>
        )}
      </motion.button>
    </div>
  );
      ...
      <AnimatePresence>
        ...
        {product && (
          <motion.div
            ...
            className="fixed z-50 left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-full max-w-4xl h-auto min-h-[350px] bg-[#FAF6EE] rounded-3xl shadow-2xl flex overflow-hidden"
          ...
          >
            <div className="relative w-[46%] shrink-0 bg-[#F1ECE3]/70 flex flex-col overflow-hidden">
              <div className="relative aspect-[4/3] overflow-hidden shrink-0 mr-4 rounded-tl-[24px] rounded-br-[24px] bg-[#F1ECE3]">
                ...
                {badgeDef && <span className={`absolute top-3 left-3 z-10 px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-md ${badgeDef.className}`}>{t(PRODUCT_BADGE_KEYS[badgeDef.id])}</span>}
              </div>
              {recommendedProducts.length > 0 && (
                <div className="w-full mt-3 px-4 pb-2 min-h-0">
                  <span className="block font-bold uppercase tracking-wider text-[#8E7A68] text-[13px] mb-2">{t('recommendedWith')}</span>
                  <div ref={(node) => { if (node) recScrollRef.current = node; }}
                    onPointerDown={onRecPointerDown} onPointerLeave={onRecPointerUp} onPointerUp={onRecPointerUp} onPointerMove={onRecPointerMove}
                    onScroll={(e) => updateRecDots(e.currentTarget)}
                    ...
                    {recommendedProducts.map((rec) => (
                      ...
                      >
                        ...
                        <div className="p-1.5 w-full">
                          <h4 className="font-sans font-medium text-[11px] text-[#231913] leading-snug line-clamp-2 pointer-events-none">{getProductName(rec)}</h4>
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            <div className="flex flex-col flex-1 min-w-0 bg-[#FAF6EE]">
              <div className="shrink-0 flex items-start justify-between gap-4 px-7 pr-16 pt-5 pb-3.5 border-b border-[#E6DFD5]/70 bg-[#FAF6EE]">
                <h3 className="font-display font-bold text-3xl text-[#231913] leading-tight">{getProductName(product)}</h3>
            ...
            <button onClick={onClose} className="absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-[#3E2F26]/80 text-white backdrop-blur-md flex items-center justify-center hover:bg-[#3E2F26] active:scale-95 transition-all shadow-lg border border-white/10" aria-label="Close">
              <X className="w-5 h-5" />
            </button>
          </motion.div>
        )}
        {product && cartButtonBlock}
      </AnimatePresence>
    );
  }
    ...
    <AnimatePresence>
      ...
      {product && (
        <motion.div
          ...
          className="fixed z-50 bottom-0 left-0 right-0 max-w-lg mx-auto bg-[#FAF6EE] rounded-t-3xl flex flex-col overflow-hidden shadow-2xl border-t border-x border-[#E6DFD5]"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="relative w-full aspect-[4/3] bg-[#F1ECE3] shrink-0 overflow-hidden">
            ...
            <div className="absolute inset-0 z-[5] overflow-hidden pointer-events-none" aria-hidden="true">
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/45 to-transparent"
                style={{ height: float.gradientHeight, opacity: float.gradientOpacity }}
              />
              <div className="absolute flex items-start justify-between gap-3"
                style={{ left: cloneLayout.left, width: cloneLayout.width, height: cloneLayout.height, bottom: float.cloneBottom, opacity: float.cloneOpacity }}
              >
                <h3 className="font-display font-bold text-2xl sm:text-3xl text-[#FAF6EE] leading-tight drop-shadow-[0_1px_3px_rgba(0,0,0,0.45)]">{getProductName(product)}</h3>
                <span className="font-bold text-2xl sm:text-3xl text-[#E9C78E] shrink-0 tracking-tight drop-shadow-[0_1px_3px_rgba(0,0,0,0.45)]">{product.price} {t('priceCurrency')}</span>
              </div>
            </div>
            <button onClick={onClose} className="absolute top-3.5 right-3.5 z-10 w-9 h-9 rounded-full bg-black/50 text-white backdrop-blur-md flex items-center justify-center hover:bg-black/70 active:scale-95 transition-all shadow-lg border border-white/10" aria-label="Close">
              <X className="w-5 h-5" />
            </button>
          </div>
