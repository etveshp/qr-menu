...
- [ ] **Ручна перевірка користувача**: повний вхід через Google в адмінку та збереження даних (потребує Google-акаунта адміна).
...
- [ ] **Ручна перевірка користувача**: збереження налаштувань реклами в адмінці (потребує входу через Google як адмін).
...
- [ ] **Ручна перевірка користувача**: повторне відкриття фото для редагування показує оригінал у масштабі 1 (потребує адмін-входу; тестові дані є: `advertising.photo_original` заповнено).
