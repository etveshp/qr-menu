...
## Фаза 11 — БЕЙДЖІ СТРАВ (гілка `feat/product-badges`)

### 11.1 Виконано
- [x] Колонка `products.badge` (text, default ''), міграція `20260907090000_product_badge.sql` + live-БД.
- [x] `Product.badge`, мапінг/upsert у lib, `/api/menu` повертає badge.
- [x] Каталог `lib/badges.ts` (new/sale/no-sugar/lent + кольори палітри) + переклади uk/hu/en.
- [x] Адмінка: радіокнопки бейджа у налаштуваннях страви (без бейджа / 4 варіанти).
- [x] Картка страви в меню показує чіп бейджа (верхній лівий кут фото).
- [x] 167 тестів (+ `lib/__tests__/badges.test.ts`), tsc, lint, build — чисто.

---

## Фаза 12 — МОВА ПО ЗАМОВЧУВАННЮ (гілка `feat/default-language`)

### 12.1 Виконано
- [x] Колонка `cafe_info.default_lang` (default 'uk'), міграція + live-БД.
- [x] Тип/мапінг/upsert/API (`defaultLang`).
- [x] Адмінка: секція «Мови» → «Мова по замовчуванню» (uk/hu/en) + збереження.
- [x] Меню: застосовує default_lang, якщо гість не обирав мову раніше.
- [x] 167 тестів, tsc, lint, build — чисто.

---

## Фаза 13 — АВТОПЕРЕКЛАД ТЕКСТІВ (гілка `feat/default-language`)

### 13.1 Виконано
- [x] `/api/translate` — серверний проксі (Google public `client=gtx`, без ключа; fallback оригінал).
- [x] Компонент `AutoTransField` (поле + бейдж коду в основній мові / кнопка-іконка перекладу для інших; session-переклади в адмінці).
- [x] Застосовано: назва/опис закладу, привітання клієнтів/адміна, назва категорії, назва/опис/інгредієнти страви. Збереження звичайними кнопками пише переклади в БД.
- [x] 167 тестів, tsc, lint, build — чисто.
...
| 2026-09-05 | Перехід між товарами в модалці: повернуто анімацію «накладання» (popLayout + key, нова картка знизу вверх); fix: `recScrollRef` не обнуляється на exit старої модалки → свайп рекомендацій після переходу працює (десктоп+mobile). 164 тести, tsc, lint — чисто | Kilo |
...
| 2026-09-14 | Гілка `feature/photo-quality`: якість фото ~2x — налаштування в `lib/image-compression.ts`, лінійна роздільність x2 (банер 2400×1350, лого 500, категорія 1200×900, страва 1200, реклама 1080×1920), WebP 0.9–0.92 на обох етапах, показ `quality={90}` + `images.qualities: [75, 90]`. Тест `image-compression` (3). 273 тести, tsc, lint, build — чисто | Kilo |
| 2026-09-14 | Сиріт/кеш фото: `storeOrKeep` додає `?v=` (cache-busting), додано `removePhotoObjects` — прибирання об'єктів зі Storage при очищенні фото та видаленні категорій/страв/банера/логотипа/реклами. Тести `photo-upload` (7). 270 тестів, tsc, lint, build — чисто; потрібен деплой | Kilo |
| 2026-09-13 | Інцидент прод (меню недоступне): корінь — `storeOrKeep` використовував `fetch(data:)`, який блокує CSP, тож фото зберігались base64 у БД; `/api/menu` виріс до 2.26 МБ (> ліміт кешу Next) і SSR головної падав. Перенесено 80 base64-значень у Storage (меню відновлено **без деплою**: API 2.26 МБ → 38 КБ), `storeOrKeep` переведено на `dataUrlToBlob`, додано guard non-JSON у `app/page.tsx`, тест `photo-upload` (3). 266 тестів, tsc, lint, build — чисто | Kilo |
| 2026-09-12 | Пост-Фаза 17 (recovery): скидання пароля для того, хто забув, більше не веде в глухий кут — `handleRecoveryToken` підтримує token_hash / PKCE `code` / implicit hash через маркер `flow=recovery`; `subscribeToAuth` прокидає `PASSWORD_RECOVERY`, `handleAuthUser` показує окремий екран зміни пароля. Автотести `recovery` (8). 263 тести, tsc, lint (0 problems), build — чисто | Kilo |
