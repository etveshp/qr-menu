...
import { computeTitleFloat, TITLE_FLOAT_GRADIENT_GAP } from '@/lib/title-float';
...
describe('computeTitleFloat', () => {
  ...
  it('is fully over the photo when scrolled by paddingTop + rowHeight', () => {
    ...
    expect(s.cloneBottom).toBe(0);
    expect(s.gradientHeight).toBe(ROW + TITLE_FLOAT_GRADIENT_GAP);
  ...
  it('cross-fades in the middle of the transition', () => {
    ...
    expect(s.cloneBottom).toBe(ROW / 2);
    expect(s.gradientHeight).toBe((ROW + TITLE_FLOAT_GRADIENT_GAP) / 2);
  });
